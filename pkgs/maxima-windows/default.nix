...
let
  ...
  rustc = callPackage ./msvc-rust.nix { inherit clang-cl; inherit (cross) windows; rustc = toolchain; };
in
(maxima.override { toolchain = toolchain; rustc = rustc; }).overrideAttrs (old: {
  buildInputs = [
